...
1. **自动翻译**：看 X / Reddit 时，把外文帖子和评论**原地**变成中文——译文直接出现在原文的位置、用原文的字体字号，看起来就像帖子本来就是中文写的。@用户名、链接、表情原样保留、可点击。悬停译文右侧出现「原文」按钮，一键切回。任何 OpenAI 兼容接口都能用，默认硅基流动 + `Qwen/Qwen3.5-35B-A3B`（快、便宜）
...
2. 「加载已解压的扩展程序」，选这个目录
3. 首次安装会自动打开设置页：选「硅基流动」，粘贴 API Key（[cloud.siliconflow.cn/account/ak](https://cloud.siliconflow.cn/account/ak)），点「测试连接」看到 ✓ 就行。评论过滤不用配置，开箱即用

## 翻译是怎么做的

- **只翻非中文**：X 的每条推文带 `lang` 属性，`zh` 直接跳过；其它按字符构成判断（`lang.js`），纯链接 / 纯 @ / 纯表情 / 太短的不发请求
- **滚到哪翻到哪**：IntersectionObserver 进视口才请求（提前 300px），无限滚动新加载的内容自动跟上；每批 6 段、3 个请求并发，设置页「高级」可调
...
翻译服务预设：硅基流动、OpenRouter、自定义（任意 `/v1/chat/completions` 兼容接口，首次使用会弹一次域名授权）。设置页「获取模型列表」拉 `GET /models` 做补全；「测试连接」会报首字延迟、tokens/s，并检查该模型是否保留占位符。
