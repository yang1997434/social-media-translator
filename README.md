...
接口是 OpenAI 兼容的 `/chat/completions`，换服务商只需改 `TR_DEFAULTS.baseUrl` 和 `manifest.json` 的 `host_permissions`。设置页填了 Key 会自动拉 `GET /models` 给模型名做补全；「测试」会检查该模型是否保留占位符。
...
- **自带 key**：`chrome.storage.sync` 里的 `apiKey` 填 OpenRouter key 就直连 OpenRouter、不限量、自己付费（设置页已不暴露这个入口，免费额度够用）。
