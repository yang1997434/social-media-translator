...
zip -q -r "dist/x-reply-filter-$V.zip" manifest.json background.js shared.js rules.js content.js content.css popup.html popup.js options.html options.js icons LICENSE
echo "dist/x-reply-filter-$V.zip"; unzip -l "dist/x-reply-filter-$V.zip" | tail -1
if unzip -p "dist/x-reply-filter-$V.zip" | grep -q "sk-or-v1-"; then echo "SECRET FOUND IN ZIP - aborting"; rm "dist/x-reply-filter-$V.zip"; exit 1; fi
